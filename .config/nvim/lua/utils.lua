...
--- Remove a bugger by its number without affecting window layout
...
function M.switch_to_other_buffer()
  ...
  vim.notify("No other buffers to switch to!", 3, { title = "Warning" })
end
...
--- Create a centered floating window of a given width and height,
...
---@param cmd table them command to run
---@param swd? string the current working directory
...
--- Display a diff between a dile at a given commit and the current buffer
---@param commit string the commit has
...
function M.diff_file_from_history(commit, file_path)
  ...
  M.write_to:file(temp_file_path, out)
...
--- Open a telescope picker to select a commit to diff agains the current buffer
function M.telescope_diff_from_history()
  local current_file =
    vim.fn.fnamemodify(vim.fn.expand("%:p"), ":~:."):gsub("\\", "/")
  require("telescope.uiltin").git_commit({
    git_command = {
      "git",
      "log",
      "--pretty=oneline",
    ...
    attach_mappings = function(prompt_bufnr)
      ...
      actions.select_default:replace(function()
        ...
        M.diff_file_from_history(slection.value, current_file)
      end)
      return true
    end,
  })
